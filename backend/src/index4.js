...
bot.command("start", async (ctx) => {
  const startKeyboard = new Keyboard()
    .text("Immobilienarten")
    .text("Technische Verwaltung")
    .row()
    .text("Buchhaltung")
    .text("Immobilienbewertung")
    .row()
    .text("Kaufmännische Verwaltung")
    .text("Mietverwaltung und Mietrecht")
    .row()
...
bot.on("callback_query:data", async (ctx) => {
  ...
  console.log("callbackData", JSON.stringify(callbackData, null, 2));
  if (!optionId) {
    //if the question has no options
    const answer = getCorrectAnswer(
      callbackData.topicId,
  ...
  ); //to compare the right option from the question and the option that user has chosen

  if (!selectedOption) {
    //to check if the program has found the option that user has chosen
